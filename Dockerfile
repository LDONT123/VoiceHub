...
# ==========================================
# 预定义各架构的构建镜像
FROM node:26-alpine AS builder-amd64
FROM node:26-alpine AS builder-arm64
FROM arm32v7/node:23-alpine AS builder-arm
...
FROM arm32v7/node:23-alpine AS runtime-arm
